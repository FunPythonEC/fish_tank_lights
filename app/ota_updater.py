...
class OTAUpdater:
    ...
    def _create_new_version_file(self, latest_version):
        ...
        with open(
            self.modulepath(self.new_version_dir + "/.version"), "w"
        ) as versionfile:
    ...
    def _install_new_version(self):
        ...
        if self._os_supports_rename():
            os.rename(
                self.modulepath(self.new_version_dir), self.modulepath(self.main_dir)
            )
        else:
